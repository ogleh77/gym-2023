    ...
    public void initialize(URL location, ResourceBundle resources) {
        Platform.runLater(() -> {
            ...
            for (Payments payment : customer.getPayments()) {
                ...
                EventHandler<MouseEvent> pending = event -> {
                    ...
                    if (payment.isPending()) {
                        try {
                            ...
                            informationAlert("Waad xidhay");
                        } catch (Exception e) {
                            errorMessage(e.getMessage());
                            throw new RuntimeException(e);
                        }
                        System.out.println("Wa la xidhay ");
                        payment.getPendingBtn().setText("Dib ufur");
                        payment.getPendingBtn().setStyle("-fx-background-color: red");
                        tableView.refresh();
                    } else {
                        System.out.println("Wa la furay");
                        payment.getPendingBtn().setText("xidh");
                        payment.getPendingBtn().setStyle("-fx-background-color: green");
                ...
                // TODO: 12/03/2023 pend during the databse
